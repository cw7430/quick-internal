DROP TABLE chat_message_alarm CASCADE;
DROP TABLE chat_room_alarm CASCADE;
...
(
    ...
    CONSTRAINT fk_alarm_users_2 FOREIGN KEY (other_user_id)
        REFERENCES users (id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_alarm_chat_room_1 FOREIGN KEY (chat_room_id)
        REFERENCES chat_room (id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT ck_alarm_type CHECK (type IN ('CREATE', 'ACCEPT', 'MESSAGE')),
...
CREATE INDEX ix_active_alarm_user_created_at
    ON alarm (my_user_id, other_user_id, chat_room_id, active_flag, created_at DESC, id DESC);

CREATE INDEX ix_deleted_alarm_created_at
    ON alarm (my_user_id, other_user_id, chat_room_id, active_flag, deleted_at DESC, id DESC);
